...
async function getResponses(survey, user, intercode, skipRelations){
    const permissions = await intercode.getPermissions(user.intercode_id, survey.base_url);
...
async function getFeedback(survey, user, intercode){
    const memberEvents = await intercode.getMemberEvents(user.intercode_id, survey.base_url);
